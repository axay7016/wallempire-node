...
const validator = Joi.object({
    sub_category_id: Joi.string().required().description(locals['category'].Post.fieldsDescription.categoryName),
    categoryName: Joi.string().description(locals['category'].Post.fieldsDescription.categoryName),
    description: Joi.string().description(locals['sub-category'].Post.fieldsDescription.description),
}).unknown();
...
const handler = async(req, res) => {
    try {
        ...
        if (payload.description && payload.description != '' && payload.description != null) {
            data = {
                description: payload.description
            }
        }
        const categoryResult = await categoryCollection.Update({ _id: ObjectId(payload.sub_category_id) }, data)
        if (categoryResult) {
            const findCategory = await categoryCollection.SelectById({ _id: ObjectId(payload.sub_category_id) }, {});
            return res.response({
