...
const validator = Joi.object({
    section_id: Joi.string().required().description(locals['category'].Post.fieldsDescription.categoryName),
...
const handler = async(req, res) => {
    try {
        ...
        const categoryResult = await categoryCollection.Update({ _id: ObjectId(payload.section_id) }, data)
        if (categoryResult) {
            const findCategory = await categoryCollection.SelectById({ _id: ObjectId(payload.section_id) }, {});
            return res.response({
