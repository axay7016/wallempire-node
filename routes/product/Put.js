...
const validator = Joi.object({
    ...
    sub_category_id: Joi.string().min(24).required().description(locals['product'].Post.fieldsDescription.category_id),
...
const handler = async (req, res) => {
    try {
        ...
        const postResult = await postCollection.Update({ _id: ObjectId(payload.product_id) }, data)
        const findPost = await postCollection.SelectById({ _id: ObjectId(payload.product_id) }, {})
        if (postResult) {
            return res.response({
